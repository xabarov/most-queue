...
class MMn_PRTY_PNZ_Cox_approx:
    ...
    def calculate_v(self):
        ...
        #                 Zs = []
        #                 for l in range(3):
        #                     Zs.append(self.Y[i][0, j]*pass_time.G[i][j, k] * pass_time.Z[i][l][j, k])
        #                     # Zs.append(self.Y[i][0, j] * pass_time.Gr[i][l][j, k])
        #                     # Zs.append(self.Y[i][0, j] * pass_time.Z[i][l][j, k])
        #                 v = self.binom_calc(v, Zs)
        #         else:
        #             for k in range(self.cols[l_tilda-1]):
        #                 Zs = []
        #                 for l in range(3):
        #                     Zs.append(self.Y[i][0, j]*pass_time.G[l_tilda][j, k] * pass_time.Z[l_tilda][l][j, k])
        #                     # Zs.append(self.Y[i][0, j]  * pass_time.Gr[l_tilda][l][j, k])
        #                     # Zs.append(self.Y[i][0, j] * pass_time.Z[l_tilda][l][j, k])
        #                 v = self.binom_calc(v, Zs)
        #         # добавим время переходов до 1 яруса
        #         for lev in range(1, i):
        #             Times = []
        #             for k in range(3):
        #                 Times.append(self.Y[i][0, j] * inter_level_mom[lev-1][k])
        #             v = self.binom_calc(v, Times)
        #
        #         # for t in range(self.n-1):
        #         #     for s in range(self.cols[0]):
        #         #         Times = []
        #         #         for k in range(3):
        #         #             if t<self.n-1:
        #         #                 Times.append(self.t[1][0, t] * self.Y[i][0, j] *0.5 *pass_time.G[1][t, s] * pass_time.Z[1][k][t, s])
        #         #             else:
        #         #                 Times.append(self.t[1][0, t] * self.Y[i][0, j] *pass_time.G[1][t, s] * pass_time.Z[1][k][t, s])
        #         #         v = self.binom_calc(v, Times)
        v1_pr = 0
        for i in range(1, self.N):
            ...
            #         for k in range(3):
            #             if t<self.n-1:
            #                 Times.append(self.Y[i][0, j] *0.5 *pass_time.G[1][t, s] * pass_time.Z[1][k][t, s])
            #             else:
            #                 Times.append(self.Y[i][0, j] *pass_time.G[1][t, s] * pass_time.Z[1][k][t, s])
            #         v = self.binom_calc(v, Times)

        # v = self.binom_calc(v, b_mom)
