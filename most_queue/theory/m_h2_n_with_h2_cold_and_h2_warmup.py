...
class Mh2h2Warmh2Cold:
    """
    Расчет численным методом Такахаси-Таками СМО M/H2/n с H2-разогревом и H2-охлаждением.
    ...
    def __init__(self, l, b, b_warm, b_cold, n, buffer=None, N=150, accuracy=1e-6, dtype="c16", verbose=False):
        ...
        for i in range(N):
            ...
            if i < n + 1:
                if i == 0:
                    self.cols.append(3)  # 00 state + cold_1 + cold_2
                else:
    ...
    def buildB(self, num):
        ...
        if num == 1:
            # to cold phases
            output[2, 1] = self.mu[0]*self.y_c[0]
            output[2, 2] = self.mu[0] * self.y_c[1]

            output[3, 1] = self.mu[1]*self.y_c[0]
            output[3, 2] = self.mu[1] * self.y_c[1]
...
if __name__ == "__main__":
    ...
    for k in range(len(b_coev)):
        ...
        tt = Mh2h2Warmh2Cold(l, b, b_w, b_c, n, buffer=buff, verbose=verbose)
        ...
        print("\nСравнение результатов расчета методом Такахаси-Таками и ИМ.\n"
              "ИМ - M/Gamma/{0:^2d} с Gamma разогревом\nТакахаси-Таками - M/H2/{0:^2d} c H2-разогревом и H2-охлаждением "
