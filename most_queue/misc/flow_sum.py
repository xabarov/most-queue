...
def test():
    ...
    cv = 0.74  # коэффициент вариации каждого потока
    ...
    # начальные моменты суммируемых потоков. В нашем случае все потоки
    # одинаково распределены
    a = []
    for i in range(n_nums):
        params1 = GammaDistribution.get_params_by_mean_and_cv(mean, cv)
    ...
    print("{0:^18s}|{1:^10s}|{2:^10s}|{3:^10s}|{4:^10s}|{5:^10s}".format("-", "a1", "a2", "a3", "a4", "cv"))
    print("-" * 80)
    ...
    for i in range(n_nums - 1):
        ...
        errors_cv.append(SummatorNumeric.get_error(cvs_num[i], cvs_sim[i]))
    ...
    ax.plot(ns, errors_cv, label="error cv", linestyle=linestyles[2])
