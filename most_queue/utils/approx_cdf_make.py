...
# H2-аппроксимация
h2_params = rd.H2_dist.get_params(b)
...
for i in tqdm(range(n)):
    val_gamma = rd.Gamma.generate_static(gamma_params[0], gamma_params[1])
    val_h2 = rd.H2_dist.generate_static(*h2_params)
    for k in range(3):
...
for i in range(dots_num):
    y_gamma.append(rd.Gamma.get_pdf(gamma_params[0], gamma_params[1], x[i]))
    y_h2.append(rd.H2_dist.get_pdf(h2_params, x[i]))
