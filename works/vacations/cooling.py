...
def run_cool_ave(qp, save_path: str = None):
    ...
    for cool_num, cool_ave in enumerate(cools):
        print(f"Start {cool_num + 1}/{len(cools)} with cooling time={cool_ave:0.3f}... ")

        b_c = calc_moments_by_mean_and_cv(cool_ave, qp["cooling"]["cv"]["base"])
        ...
        w1_rel_errors.append(calc_rel_error_percent(sim_results["w"][0], num_results["w"][0]))
    ...
    if save_path:
        wait_time_save_path = os.path.join(save_path, "w1_vs_cool_ave.png")

        plot_w1(cools, w1_num, w1_sim, x_label="Cooling Average", save_path=wait_time_save_path)
...
def run_cool_cv(qp, save_path: str = None):
    ...
    for cool_num, cool_cv in enumerate(cools):
        print(f"Start {cool_num + 1}/{len(cools)} with cooling cv={cool_cv:0.3f}... ")

        b_c = calc_moments_by_mean_and_cv(qp["cooling"]["mean"]["base"], cool_cv)
    ...
    if save_path:
        wait_time_save_path = os.path.join(save_path, "w1_vs_cool_cv.png")

        plot_w1(cools, w1_num, w1_sim, x_label="Cooling CV", save_path=wait_time_save_path)

        w1_error_save_path = os.path.join(save_path, "w1_error_vs_cool_cv.png")

        plot_w1_errors(cools, w1_rel_errors, x_label="Cooling CV", save_path=w1_error_save_path)

        cool_probs_save_path = os.path.join(save_path, "cooling_probs_vs_cool_cv.png")
