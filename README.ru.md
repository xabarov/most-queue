...
| Приоритеты | M/G/1 PR/NP мультикласс, M/G/c PR/NP, M/Ph/c PR; **RDR** M/M/k и M/PH/k мультикласс (точный + RDR-A), дисперсия отклика по классам; **накапливаемый приоритет** (Клейнрок/APQ), приоритетный Erlang-A (нетерпение), приоритеты MMAP/PH/1 (NP/PR/RS), retrial с приоритетным классом, preemptive repeat | точный / RDR / CTMC / инвариантная аппроксимация |
...
| Надёжность | M/G/1 и M/M/c с отказами и ремонтами, machine repair problem (резерв, R ремонтников), working breakdowns, катастрофы с фазой ремонта, retrial с ненадёжным прибором | Avi-Itzhak–Naor / точная CTMC / birth-death |
...
| Сети | открытые (декомпозиция, точный Джексон, QNA с двухмоментными потоками, MAP-вход), закрытые (точный MVA / свёртка Бьюзена / Швейцер), мультиклассовые BCMP, G-сети (Геленбе, мультикласс), тандемы с блокировками (конечные буферы), fork-join станции, нестационарные λ(t), приоритеты, отрицательные заявки, оптимизация маршрутизации | декомпозиция / product form / MVA |
...
- 🗺 [Планы развития](docs/epics/README.md) и [обзор трендов](docs/research/queueing-trends-2026.md) — что дальше (queueing-inventory, сети с блокировками, расширения fork-join)
