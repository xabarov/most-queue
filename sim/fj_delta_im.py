...
from fj_im import SmoFJ, SubTask, Task
...
class SmoFJDelta(SmoFJ):
    ...
    def task_arrival(self):
        ...
        if self.buffer:  # ограниченная длина очереди
            if not self.is_SJ:
                if len(self.queue) + self.num_of_parts - 1 > self.buffer + self.free_channels:
                    self.dropped += 1
                    is_dropped = True
            else:
                if self.free_channels == 0 and len(self.queue) + self.num_of_parts - 1 > self.buffer:
                    self.dropped += 1
                    is_dropped = True

        if not is_dropped:
            self.served_subtask_in_task[Task.task_id] = 0
            t = Task(self.num_of_parts, self.ttek)
            ...
            if not self.is_SJ:  # Fork-Join discipline
                ...
                if self.free_channels < self.num_of_parts:
                    self.queue.append(t.subtasks[0])
                else:
            ...
            for i in range(1, self.num_of_parts):
    ...
    def serving(self, c):
        ...
        if not self.is_SJ:
            ...
            if self.served_subtask_in_task[end_ts.task_id] == self.num_of_parts:
                self.served += 1
