...
- [ ] **Баг GIM1Calc**: 2-й и 3-й моменты ожидания расходятся и с симуляцией, и с
      QBD-решателем PhPh1Calc (обнаружено 2026-07-08 при кросс-валидации EPIC-003:
      Erlang2/M/1, w2: сим 4.05 / QBD 4.08 / GIM1 3.10; для GI/M/1 верно
      w_k = k!·σ/(μ(1−σ))^k). Проверить формулы старших моментов в `fifo/gi_m_1.py`
      (вероятно, и `gi_m_n.py`).
