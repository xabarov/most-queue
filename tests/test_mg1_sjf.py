"""
M/G/1 SJF (non-preemptive SPT): MG1SjfCalc vs SizeBasedQsSim.
"""
...
from most_queue.theory.srpt import MG1SjfCalc
...
def test_mg1_sjf_h2():
    """MG1SjfCalc vs SizeBasedQsSim(SJF): H2 service, rho=0.7, cv=1.2."""
    ...
    sim = SizeBasedQsSim(NUM_OF_CHANNELS, discipline="SJF", verbose=False)
    sim.generator = np.random.default_rng(92001)
...
def test_mg1_sjf_gamma():
    """MG1SjfCalc vs SizeBasedQsSim(SJF): Gamma service, rho=0.7, cv=0.8."""
    ...
    sim = SizeBasedQsSim(NUM_OF_CHANNELS, discipline="SJF", verbose=False)
    sim.generator = np.random.default_rng(92002)
