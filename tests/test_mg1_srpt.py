"""
M/G/1 SRPT: MG1SrptCalc vs SizeBasedQsSim (Schrage-Miller 1966).
"""
...
def test_mg1_srpt_h2():
    """MG1SrptCalc vs SizeBasedQsSim(SRPT): H2 service, rho=0.7, cv=1.2."""
    ...
    sim = SizeBasedQsSim(NUM_OF_CHANNELS, discipline="SRPT", verbose=False)
    sim.generator = np.random.default_rng(91001)
...
def test_mg1_srpt_gamma():
    """MG1SrptCalc vs SizeBasedQsSim(SRPT): Gamma service, rho=0.7, cv=0.8."""
    ...
    sim = SizeBasedQsSim(NUM_OF_CHANNELS, discipline="SRPT", verbose=False)
    sim.generator = np.random.default_rng(91002)
